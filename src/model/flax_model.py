# Based on https://raw.githubusercontent.com/huggingface/transformers/v4.40.1/src/transformers/models/gpt2/modeling_flax_gpt2.py
...
from transformers import FlaxPreTrainedModel, GPT2Config
...
class FlaxGPT2Block(nn.Module):
    ...
    def setup(self):
        ...
        self.ln_1 = nn.LayerNorm(epsilon=self.config.layer_norm_epsilon, dtype=self.dtype, use_bias=False)
        self.attn = FlaxGPT2Attention(self.config, dtype=self.dtype)
        self.ln_2 = nn.LayerNorm(epsilon=self.config.layer_norm_epsilon, dtype=self.dtype, use_bias=False)

        if self.config.add_cross_attention:
            self.crossattention = FlaxGPT2Attention(
                config=self.config, dtype=self.dtype, causal=False, is_cross_attention=True
            )
            self.ln_cross_attn = nn.LayerNorm(epsilon=self.config.layer_norm_epsilon, dtype=self.dtype, use_bias=False)
...
class FlaxGPT2Module(nn.Module):
    ...
    def setup(self):
        ...
        self.ln_f = nn.LayerNorm(epsilon=self.config.layer_norm_epsilon, dtype=self.dtype, use_bias=False)
...
class FlaxGPT2LMHeadModule(nn.Module):
    ...
    def setup(self):
        ...
        self.dummy = self.param('dummy', jax.random.normal, (5,))  # Dummy variable
    ...
    def __call__(
        ...
        if self.config.tie_word_embeddings:
            ...
            lm_logits = self.lm_head.apply({"params": {"kernel": shared_kernel}}, hidden_states) + self.dummy.sum() * 0
        else:
